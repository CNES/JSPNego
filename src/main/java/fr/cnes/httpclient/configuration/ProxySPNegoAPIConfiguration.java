...
public enum ProxySPNegoAPIConfiguration {
    ...
    /**
     * Set this to the name of the ticket cache that contains user's TGT. 
     * If this is set, useTicketCache must also be set to true; Otherwise a configuration error 
     * will be returned. Default is environment variable KRB5CCNAME
     */
    ...
    RENEW_TGT("renewTGT", ""),
    /**
     * Set this to true if you do not want to be prompted for the password if credentials can not 
     * be obtained from the cache, the keytab, or through shared state.
     * (Default is false) If set to true, credential must be obtained through cache, keytab, or 
     * shared state. Otherwise, authentication will fail.
     */
     ...
     * For isInitiator being false, if principal is "*", the KeyTab stored can be used by anyone, 
     * otherwise, it's restricted to be used by the specified principal only.
     */
    STORE_KEY("storeKey", "true"),
    /**
     * The name of the principal that should be used. 
     * The principal can be a simple username such as "testuser" or a service name such as 
     * "host/testhost.eng.sun.com". You can use the principal option to set the principal when 
     * there are credentials for multiple principals in the keyTab or when you want a specific 
     * ticket cache only. The principal can also be set using the system property 
     * sun.security.krb5.principal.
     */
    PRINCIPAL("principal", System.getenv("sun.security.krb5.principal")),
    /**
     * Set this to true, if initiator. 
     * Set this to false, if acceptor only. Default is true.
     */
     ...
     * @see {@link https://docs.oracle.com/javase/8/docs/jre/api/security/jaas/spec/com/sun/security/auth/module/Krb5LoginModule.html}
     */    
    public static boolean isValid(StringBuilder error) {
        ...
        if(ProxySPNegoAPIConfiguration.SERVICE_PROVIDER_NAME.getValue().isEmpty()) {
            validation.append(ProxySPNegoAPIConfiguration.SERVICE_PROVIDER_NAME.getKey()).append(" must be set");
            isValid = false;
        }        
